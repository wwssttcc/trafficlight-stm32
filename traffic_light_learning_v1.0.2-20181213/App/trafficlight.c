	...
	for(i = 0; i < 3; i++)
		...
		else
		{
			if(light_info->light == red)
			{
				...
				info_msg("红绿灯学习机：dir %d to %d:红灯剩余%02d秒 %d %d\r\n", dir, to, light_info->last_seconds, ret, light_info->check);
			}
			else if(light_info->light == green)
			{
				light_info->light = 'G';
				light_info->next_light = 'Y';
				info_msg("红绿灯学习机：dir %d to %d:绿灯剩余%02d秒 %d %d\r\n", dir, to, light_info->last_seconds, ret, light_info->check);
			}
			else
			{
				light_info->light = 'Y';
				light_info->next_light = 'R';
				info_msg("红绿灯学习机：dir %d to %d:黄灯剩余%02d秒 %d %d\r\n", dir, to, light_info->last_seconds, ret, light_info->check);
			}
