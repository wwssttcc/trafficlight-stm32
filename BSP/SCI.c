...
#if 1
void USART1_IRQHandler(void)                	//RS232 串口1中断服务程序
{
	u8 Res;
...
void USART1_IRQHandler(void)                	//RS232 串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		ringBufWrite(USART_ReceiveData(USART1));
...
#else
#if 0
void USART2_IRQHandler(void)                	//RS485 串口2中断服务程序
{
	u8 Res;
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		ringBufWrite(USART_ReceiveData(USART2));
...
#if 0
void USART3_IRQHandler(void)                	//TTL 串口3中断服务程序
{
	u8 Res;
//#if SYSTEM_SUPPORT_OS 		//如果SYSTEM_SUPPORT_OS为真，则需要支持OS.
	OSIntEnter();  
...
void USART3_IRQHandler(void)                	//RS232 串口1中断服务程序
{
	OSIntEnter();  
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		ringBufWrite(USART_ReceiveData(USART3));
    while (USART_GetFlagStatus(USART3, USART_FLAG_TXE) == RESET);
  } 
	OSIntExit();  											 
...
void RS485_test(void)
{
	...
	{
		RS485_REC_Flag = 0;
		RS485_Send_Data((unsigned char *)RS485_buff,RS485_rec_counter);
		RS485_rec_counter = 0;
	}
}
